...
// Add these lines AFTER the filter parameter definitions and BEFORE the SQL query:

$sortSName = isset($_GET['sortSName']) ? $_GET['sortSName'] : '';
$sortRLevel = isset($_GET['sortRLevel']) ? $_GET['sortRLevel'] : '';
$sortFaculty = isset($_GET['sortFaculty']) ? $_GET['sortFaculty'] : '';

// Check if both filters are selected
$hasSName = !empty($sortSName);
$hasRLevel = !empty($sortRLevel);
$hasFaculty = !empty($sortFaculty);

// ✅ ADD THESE LINES HERE - Initialize counters BEFORE the main loop
$totalHighMoodRiskCount = 0;
$totalHighDassRiskCount = 0;

// Count PA based on filter
if ($hasFaculty) {
    $stmtPaCount = $conn->prepare("
        SELECT COUNT(*) as paCount
        FROM staff
        WHERE staffRole = 'PENASIHAT AKADEMIK'
        AND staffFaculty = ?
    ");
    $stmtPaCount->bind_param("s", $sortFaculty);
} else {
    $stmtPaCount = $conn->prepare("
        SELECT COUNT(*) as paCount
        FROM staff
...
$stmtPaCount->execute();
$resultPaCount = $stmtPaCount->get_result();
$paCountData = $resultPaCount->fetch_assoc();
$paCount = $paCountData['paCount'];

// Count students based on filter (students under filtered PAs)
if ($hasFaculty) {
    $stmtStudentCount = $conn->prepare("
        SELECT COUNT(s.studentId) as studentCount
        FROM student s
        JOIN staff st ON s.staffId = st.staffId
        WHERE st.staffRole = 'PENASIHAT AKADEMIK'
        AND st.staffFaculty = ?
    ");
    $stmtStudentCount->bind_param("s", $sortFaculty);
} else {
    $stmtStudentCount = $conn->prepare("
...
$resultStudentCount = $stmtStudentCount->get_result();
$studentCountData = $resultStudentCount->fetch_assoc();
$studentCount = $studentCountData['studentCount'];

// First, get all PA data (with optional faculty filter)
if ($hasFaculty) {
    $stmt = $conn->prepare("
        SELECT * FROM staff 
        WHERE staffRole = 'PENASIHAT AKADEMIK'
...
// Get student details and their mood & stress and dass details
$PADetails = [];
foreach ($data as $row) {
    $paId = $row['staffId'];
    ...
    // Count Student Assigned to that PA
    $stmtCountStudentAssigned = $conn->prepare("
        SELECT COUNT(*) as studentAssigned
        FROM student
        WHERE staffId = ?
    ");
    ...
    $studentAssigned = $resultCountStudentAssigned->fetch_assoc();

    // Get last 7 days
    $last7Days = (new DateTime())->modify('-7 days')->format('Y-m-d H:i:s');

    // Count student contacted (Last 7 days)
    $stmtCountContactedStudent = $conn->prepare("
        SELECT COUNT(DISTINCT studentId) as contactedStudent, 
               COUNT(note) as notesRecorded
        FROM contactNote
        WHERE staffId = ? AND
        datetimeRecord >= ?
    ");
    $stmtCountContactedStudent->bind_param("is", $paId, $last7Days);
    $stmtCountContactedStudent->execute();
    $resultCountContactedStudent = $stmtCountContactedStudent->get_result();
    $contactedStudentLast7Days = $resultCountContactedStudent->fetch_assoc();

    // Get all student details under each PA
    ...
    foreach ($getAllStudentData as $rowStudent) {
        ...
        $trendMoodMonth = $monthlyMoodComparison['negative']['trend'] ?? null;
        $trendStressMonth =  $monthlyStressComparison['high']['trend'] ?? null;

        if ($trendMoodMonth == "increasing" && $trendStressMonth == "increasing") {
            $countHighRiskMood++;
            $totalHighMoodRiskCount++;
        }
        ...
        $allStudentMoodData[] = [
            ...
            'noteCount' => $noteCount,
            // ✅ NEW: Add WEEKLY and MONTHLY comparison data
            'weeklyComparison' => [
                'mood' => $weeklyMoodComparison,
                'stress' => $weeklyStressComparison,
                'period' => [
            ...
            'monthlyComparison' => [
                ...
                'stress' => $monthlyStressComparison,
                'period' => [
    ...
    ");
    $stmtGetAllStudentDass->bind_param("i", $paId);
    ...
    foreach ($getAllStudentDassData as $rowDass) {
        $studentId = $rowDass['studentId'];
        ...
        $allStudentDassData[] = [
            'studentId' => $studentId,
            'matricNo' => $rowDass['matricNo'],
            'studentName' => $rowDass['studentName'],
    ...
    $PADetails[] = [
        "PAData" => $row,
        "period" =>$period,
        "studentAssigned" => $studentAssigned['studentAssigned'],
        "contactedStudent" => $contactedStudentLast7Days['contactedStudent'],
        "noteRecorded" => $contactedStudentLast7Days['notesRecorded'],
        "highRiskMood" => $countHighRiskMood ?? 0,
        "highRiskDass" => $countHighDassMood ?? 0,
    ];
}

// ✅ Sort the array using usort
usort($PADetails, function($a, $b) use ($hasSName, $hasRLevel, $sortSName, $sortRLevel) {
    // Priority 1: Risk Level (if selected)
    if ($hasRLevel) {
        if ($sortRLevel === 'low') {
            // High-Risk (Mood & Stress) - highest to lowest
            $riskA = $a['highRiskMood'];
            $riskB = $b['highRiskMood'];
        } else {
            // High-Risk (DASS) - highest to lowest
            $riskA = $a['highRiskDass'];
            $riskB = $b['highRiskDass'];
        }
        
        // Sort by risk (descending - highest first)
        if ($riskB !== $riskA) {
            return $riskB - $riskA;
        }
    }
    
    // Priority 2: PA Name (if selected)
    if ($hasSName) {
        return $sortSName === 'az' 
            ? strcmp($a['PAData']['staffName'], $b['PAData']['staffName'])
            : strcmp($b['PAData']['staffName'], $a['PAData']['staffName']);
    }
...
$dashboardData = [
    ...
    'totalHighMoodRiskCount' => $totalHighMoodRiskCount ?? 0,
    'totalHighDassRiskCount' => $totalHighDassRiskCount ?? 0
];

echo json_encode([
    "success" => true,
    "PAData" => $user,
    "allPADetails" => $PADetails,
