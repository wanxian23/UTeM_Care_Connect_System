...
$tab = isset($_GET['tab']) ? $_GET['tab'] : 'studentList';
...
$query = "SELECT * FROM student WHERE staffId = ?";
...
if ($tab === 'studentList') {
    // ✅ ADD THESE LINES - Initialize variables BEFORE the loop
    $moodRecordedCount = 0;
    $dassRecordedCount = 0;
    $highMoodRiskCount = 0;
    $allStudentMoodData = [];
    ...
    usort($allStudentMoodData, function($a, $b) use ($hasSName, $hasRLevel, $sortSName, $sortRLevel) {
        if ($hasSName && $hasRLevel) {
            // Sort by risk level first
            $riskOrder = ['high' => ['High' => 1, 'Need Attention' => 2, 'Low' => 3],
                          'medium' => ['Need Attention' => 1, 'High' => 2, 'Low' => 3],
                          'low' => ['Low' => 1, 'Need Attention' => 2, 'High' => 3]];
            
            $riskA = $riskOrder[$sortRLevel][$a['riskIndicator']] ?? 999;
            $riskB = $riskOrder[$sortRLevel][$b['riskIndicator']] ?? 999;
            ...
            return $sortSName === 'az' 
                ? strcmp($a['studentName'], $b['studentName'])
                : strcmp($b['studentName'], $a['studentName']);
            ...
            $riskOrder = ['high' => ['High Risk' => 1, 'Need Attention' => 2, 'Low Risk' => 3],
                          'medium' => ['Need Attention' => 1, 'High Risk' => 2, 'Low Risk' => 3],
                          'low' => ['Low Risk' => 1, 'Need Attention' => 2, 'High Risk' => 3]];
    ...
    // DASS TAB - Follow EXACT React logic
    $allStudentDassData = [];
    ...
    foreach ($data as $row) {
        ...
        // ✅ FOLLOW EXACT REACT LOGIC - Initialize as "Low Risk" then calculate
        $riskStatus = "Low Risk";
        ...
        if ($dassId && $studentDassDataRow['status'] !== "Pending") {
            ...
            // ✅ CALCULATE RISK - EXACT REACT LOGIC from DassTable
            $normalCount = 0;
            $mildCount = 0;
            $moderateCount = 0;
            $severeCount = 0;
            ...
            // This follows YOUR React logic EXACTLY:
        ...
        // Always add student to the array
        $allStudentDassData[] = [
            'studentId' => $studentId,
            'matricNo' => $row['matricNo'],
            'studentName' => $row['studentName'],
            ...
            'riskIndicator' => $riskIndicator, // ✅ Now matches React calculation
            'contactRecord' => $contactSent,
            'noteRecord' => $noteRecord,
    ...
    // Sort the array in PHP
    usort($allStudentDassData, function($a, $b) use ($hasSName, $hasRLevel, $sortSName, $sortRLevel) {
        if ($hasSName && $hasRLevel) {
            // Sort by risk level first
            $riskOrder = ['high' => ['Critical' => 1, 'High' => 2, 'Medium' => 3, 'Low' => 4],
                          'medium' => ['Medium' => 1, 'High' => 2, 'Critical' => 3, 'Low' => 4],
