...
$stmtAllPaCount = $conn->prepare("
    SELECT COUNT(*) as allPACount
    FROM staff
    WHERE staffRole = 'PENASIHAT AKADEMIK'
");
$stmtAllPaCount->execute();
$resultAllPaCount = $stmtAllPaCount->get_result();
$allPaCountData = $resultAllPaCount->fetch_assoc();
$paCount = $allPaCountData['allPACount'];

// Count All Student
$stmtAllStudentCount = $conn->prepare("
    SELECT COUNT(*) as allStudentCount
    FROM student
");
$stmtAllStudentCount->execute();
$resultAllStudentCount = $stmtAllStudentCount->get_result();
$allStudentCountData = $resultAllStudentCount->fetch_assoc();
$studentCount = $allStudentCountData['allStudentCount'];

// Get all the data of PA
$stmtGetAllPADetails = $conn->prepare("
    SELECT * 
    FROM staff
    WHERE staffRole = 'PENASIHAT AKADEMIK'
    ORDER BY staffName ASC
"); 
$stmtGetAllPADetails->execute();
$resultGetAllPADetails = $stmtGetAllPADetails->get_result();
$allPADetails = $resultGetAllPADetails->fetch_all(MYSQLI_ASSOC);

// Get student details
// And their mood & stress and dass details
$totalHighMoodRiskCount = 0;
$totalHighDassRiskCount = 0;
$PADetails = [];
foreach ($allPADetails as $row) {
    ...
    // Get last 1 month
    $last1Month = (new DateTime())->modify('-1 month')->format('Y-m-d H:i:s');

    // Count student contacted (Last 7 days)
    $stmtCountContactedStudent = $conn->prepare("
        SELECT COUNT(*) as contactedStudent, COUNT(note) as notesRecorded
        FROM contactNote
        WHERE staffId = ? AND
        datetimeRecord >= ?
    ");
    ...
    // Get all student details under the each PA
