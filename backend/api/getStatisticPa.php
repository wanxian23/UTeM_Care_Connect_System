...
if ($_SERVER['REQUEST_METHOD'] === 'OPTIONS') {
    // Allow the frontend origin (or use * in dev, but restrict in prod)
    header("Access-Control-Allow-Origin: http://localhost:3000");
    header("Access-Control-Allow-Methods: GET, POST, OPTIONS");
    header("Access-Control-Allow-Headers: Authorization, Content-Type");
    header("Access-Control-Max-Age: 86400"); // Cache preflight for 1 day
    exit; // Important: stop here for preflight
}
...
if (empty($paId)) {
    $stmtGetStudentDetails = $conn->prepare("
        ...
        AND staffId = ?
    ");
    $stmtGetStudentDetails->bind_param("ii", $studentId, $staffId);
} else {
    $stmtGetStudentDetails = $conn->prepare("
        SELECT * FROM Student
        WHERE studentId = ?
        AND staffId = ?
    ");
    $stmtGetStudentDetails->bind_param("ii", $studentId, $paId);
}
...
// StudentId
$userId = $studentDetails['studentId'] ?? null;
