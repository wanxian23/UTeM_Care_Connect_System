...
if (empty($query)) {
    $stmt = $conn->prepare("
        SELECT * FROM student WHERE staffId = ?
    ...
    $stmt = $conn->prepare("
        SELECT * FROM student 
        WHERE staffId = ? 
        AND (studentName LIKE ? OR matricNo LIKE ?)
        ORDER BY studentName ASC
    ");
    $stmt->bind_param("iss", $staffId, $searchTerm, $searchTerm);
}
...
// Return appropriate data based on tab
if ($tab === 'studentList') {
    echo json_encode([
        "success" => true,
