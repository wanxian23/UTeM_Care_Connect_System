...
function TableDataCounselling() {
    ...
    return(
        <>
            ...
            <PAInformation allPADetails={allPADetails} dashboardData={dashboardData}/>
            <Footer />
        </>
    );
}
...
function PAInformation({allPADetails, dashboardData}) {
    ...
    const handleExportCSV = (format) => {
        if (format === 'csv') {
            ...
            exportMoodSummaryPDF(filteredPAData);
        }
    };
    ...
    const exportMoodSummaryCSV = (allPADetails) => {
        ...
        const headerSection = [
            ...
            ["", ""],
            ["Summary", ""],
            ["Total Penasihat Akademik", dashboardData?.paCount ?? 0],
            [
                "Total Students at High Emotional Risk",
                `${dashboardData?.totalHighMoodRiskCount ?? 0} | ${dashboardData?.studentCount ?? 0}`
            ],
            [
                "Total Students at High DASS Risk",
                `${dashboardData?.totalHighDassRiskCount ?? 0} | ${dashboardData?.studentCount ?? 0}`
            ],
            ["Export Date", today],
            ["", ""]
        ];
        ...
        const csvContent = [
            ...
            link.href = url;
            link.download = `PA_Student_Mood_DASS_Summary_${today}.csv`;
            link.click();
        ...
        // PDF Export Function
        const exportMoodSummaryPDF = (allPADetails) => {
            if (!allPADetails || allPADetails.length === 0) return;
            ...
            doc.text('PA x Student Mood & DASS Summary Report', 14, 20);
            ...
            doc.text(`Total Penasihat Akademik: ${dashboardData?.paCount ?? 0}`, 14, yPos);
            yPos += 7;
            
            doc.text(`Students at High Emotional Risk: ${dashboardData?.totalHighMoodRiskCount ?? 0} / ${dashboardData?.studentCount ?? 0}`, 14, yPos);
            yPos += 7;
            
            doc.text(`Students at High DASS Risk: ${dashboardData?.totalHighDassRiskCount ?? 0} / ${dashboardData?.studentCount ?? 0}`, 14, yPos);
            yPos += 7;
            ...
            // Table headers
            const tableHeaders = [
                ['PA No', 'PA Name', 'Faculty', 'Students', 'Period', 'Mood Risk', 'DASS Risk', 'Contacted', 'Case Notes Recorded']
            ];

            // Table rows
            const tableRows = allPADetails.map(PADetails => [
                PADetails.PAData.staffNo,
            ...
            doc.save(`PA_Student_Mood_DASS_Summary_${today}.pdf`);
        };
